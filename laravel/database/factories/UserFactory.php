...
class UserService
{
    ...
    // BÚSQUEDA Y LISTADO
    // Obtiene los usuarios paginados aplicando filtros por rol, estado, responsable o término de búsqueda
    ...
    // CREACIÓN DE USUARIO Y NOTIFICACIÓN
    // Registra la cuenta principal, genera credenciales, asocia su perfil y envía el email de bienvenida
    ...
    public function crear(array $datos): User
    {
        $usuario = DB::transaction(function () use ($datos) {
            ...
            // Almacenar contraseña temporal en el objeto retornado en texto plano para el envío del correo
        ...
        // Lógica de notificaciones (Dependiendo de si es menor, mayor, o staff)
    ...
    // ACTUALIZACIÓN DE USUARIO Y PERFIL
    // Modifica los datos de acceso generales y la información específica según el rol asignado
    ...
    // Gestiona el borrado del sistema (lógico por defecto para mantener historial, o físico)
    ...
    // Funciones auxiliares para crear datos predeterminados en el momento del registro
    ...
    // GESTIÓN INTERNA DE PERFILES
    // Métodos privados (Helpers) encargados de crear o actualizar los datos específicos de rol
    ...
    private function crearPerfil(User $usuario, array $datos): void
    {
        if ($usuario->rol === 'entrenadora') {
            ...
            // Creación del tutor legal si es menor de edad
    ...
    private function actualizarPerfil(User $usuario, array $datos): void
    {
        if ($usuario->rol === 'entrenadora' && $usuario->entrenador) {
            ...
            // Actualización o eliminación del tutor legal basado en la edad
